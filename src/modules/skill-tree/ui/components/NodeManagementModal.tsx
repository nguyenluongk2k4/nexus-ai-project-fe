  ...
  return (
    <div>
      ...
      >
        ...
        {/* Avatar */}
        <div className={`w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold flex-shrink-0 ${config.bg}`}>
          {node.type?.[0]?.toUpperCase() || 'N'}
        </div>
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-white text-lg font-bold flex-shrink-0 ${
          alt.type === 'knowledge' ? 'bg-orange-500' : 'bg-indigo-500'
        }`}>
          {alt.name?.[0]?.toUpperCase() || alt.type?.[0]?.toUpperCase() || 'A'}
        </div>
            ...
            <div className="mb-6">
              ...
              <div className="flex items-start gap-4">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-white text-xl font-bold flex-shrink-0 ${
                  node.type === 'ability' || node.type === 'specialization' ? 'bg-fuchsia-500' :
                  node.type === 'skill' ? 'bg-pink-500' : 'bg-indigo-500'
                }`}>
                  {node.type?.[0]?.toUpperCase() || 'N'}
                </div>
